...
exports.run = function(bot, message, args) {
  ...
  message.delete(10000).catch(console.error);
    if (!message.member.voiceChannel) return message.reply("**:bangbang: Sana bağlanmam için ilk önce sesli bir kanala katılmalısın. :bangbang:** HÜSEYİN EMRE ŞENS").then(m => m.delete(15000)).catch(console.error);
    else {
        if (!args[0] || args[0] === "help" || args[0] === "yardım") {
            message.reply("**\n \n  🎵 Radyo İstasyonları 🎵  \n Youtube HÜSEYİN EMRE ŞEN  \n \n 1 = Fenomen \n 2 = FenomenTürk \n 3 = KralPop \n 4 = Virgin Radio\n \n Açmak İçin cr!radyo <numara> \n Kapatmak İçin cr!radyo kapat**").then(m => m.delete(40000)).catch(console.error);
